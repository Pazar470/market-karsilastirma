...
import { getSessionFromCookie } from '@/lib/user-session';
...
export async function deleteSmartAlarm(id: string) {
    const session = await getSessionFromCookie();
    if (!session) return { success: false, error: 'Oturum gerekli' };
    try {
        const alarm = await prisma.smartAlarm.findFirst({
            where: { id, userId: session.userId },
        });
        if (!alarm) return { success: false, error: 'Alarm bulunamadı' };
        await prisma.smartAlarm.delete({ where: { id } });
...
export async function createSmartAlarm(prevStateOrFormData: unknown, formDataOrUndefined?: FormData) {
    const session = await getSessionFromCookie();
    if (!session) return { success: false, error: 'Oturum gerekli. Lütfen giriş yapın.' };
    ...
    if (!categoryId || !targetPrice) return { success: false, error: 'Missing required fields' };
...
export async function excludeProductFromAlarm(alarmId: string, productId: string) {
    ...
    try {
        const alarm = await prisma.smartAlarm.findFirst({
            where: { id: alarmId, userId: session.userId },
        });
