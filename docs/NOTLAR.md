...
- **Senaryo:** İstatistik sitesi değiliz; o gün taradığımız marketlerde **satışta olan** ürünleri gösteriyoruz. Dün A101’de vardı bugün yoksa kullanıcıya göstermiyoruz (admin onayı gerekmez). Aynı ürün 25 gün sonra tekrar markette satıştaysa ve bizde kategori yolu yoksa (mapping’te de yoksa) o zaman admin sayfasına düşer; orada kategori yolu seçilir. Admin bekleyen listesi de **sadece son 24 saatte fiyatı olan** null ürünlerle doldurulur (göstereceğimiz ürünler).
...
- **ODS = bir kere, sonra sadece Mapping + admin:** (1) **Bir kere** (veya ODS değişince): `npx tsx scripts/clean-and-import-ods.ts "docs/tum_urunler_manuel.ods"` — Category + Mapping + Manuel + ODS’te eşleşen ürünlerin categoryId’si yazılır; diğer ürünlere dokunulmaz. (2) **Her tam taramadan sonra** sadece **Mapping senkronu** otomatik çalışır: Tarama tek yol üzerinden yapılır — `run-full-scan-offline.ts` (bölüm 10). Bu script indirme → yükleme → Mapping senkronu → alarm’ı sırayla yapar. Mapping’te olan (market, kod) için hâlâ categoryId=null olan ürünler o mapping’in categoryId’si ile güncellenir. Yeni (market, kod) veya manuel kategoride yeni ürün → admin onayına düşer. ODS’i sonradan güncellediysen: tekrar `clean-and-import-ods` veya sadece ürün yolları için `npx tsx scripts/apply-ods-product-categories.ts "docs/tum_urunler_manuel.ods"`. Manuel sadece Mapping senkronu: `npx tsx scripts/sync-mapping-to-null-products.ts`.
...
- **Şok tarama:** Kategori başına en fazla 200 sayfa (SOK_MAX_PAGES_PER_CATEGORY); tam ürün çekmek için yeterli. Kategori URL’si `sok_categories.json` içindeki `cat.url` kullanılır (çift slash `//` domain sonrasında düzeltilir). Şok kategori sayfalarında __NEXT_DATA__ yok; ürünler HTML’den `a[href*="-p-"]` + fiyat regex ile çıkarılır. Pagination: sayfa 1, 2, … boş dönene kadar taranır. Test: `npx tsx scripts/test-sok-scraper.ts` (birkaç kategori için sayfa 1/2 ürün sayısı ve engelleme kontrolü).

---

## 9. Tatbikat: Tam senaryo (kategori, mapping, manuel, admin)

**Otomasyon:** Tam taramayı çalıştırdığında (bölüm 10’daki tek komut: `run-full-scan-offline.ts`) akış zaten otomatik: İndirme → Upload → **Mapping senkronu** (null ürünler mapping’e göre dolar) → Alarm. Sen ekstra bir şey çalıştırmıyorsun.
...
## 13. Bilgisayar yavaşlığı: odt2txt, Git, Cursor

- **odt2txt / sh (Git mingw64):** Projede ODS dosyası **kod tarafında** sadece Python (pandas + odfpy) ile okunur; `odt2txt` hiç kullanılmıyor. `odt2txt` süreçleri **Git** kaynaklıdır: Git, `.ods` dosyaları için diff alırken (küçük metin göstermek için) `odt2txt` çağırır. Cursor/IDE sürekli `git status` / `git diff` çalıştırdığı için repoda `.ods` varsa sürekli 8–10 odt2txt + sh açılıyor.
- **Proje çözümü:** Repo köküne **`.gitattributes`** eklendi: `*.ods binary`, `*.odt binary`. Böylece bu repoda Git ODS dosyalarında diff çalıştırmaz, odt2txt tetiklenmez. Deploy sonrası bu dosyayı commit edip push et; yerelde de aynı projeyi açtığında geçerli olur.
- **Global Git ayarı:** Eğer yine odt2txt çıkıyorsa, bilgisayarda **genel** Git ayarı var demektir. `git config --global --list` ile `diff.odf.textconv` veya benzeri var mı bak; varsa kaldır veya bu projede `.gitattributes` ile ezmiş olursun.
- **Cursor çok örnek:** 14 Cursor penceresi/süreç açık kalmış olabilir. Görev Yöneticisi’nden gereksiz “Cursor” / “Code” süreçlerini kapat. Cursor ayarlarında “Startup: Open previous folder” / “Restore windows” kapatırsan her açılışta eski 14 pencere açılmaz.
- **Özet:** ODS taramada okunmaz; sadece sen `clean-and-import-ods` çalıştırdığında Python ile okunur. Yavaşlık Git’in ODS için odt2txt çağırmasından. `.gitattributes` bunu bu repoda kapatır.

---

## 14. Kategori kaçırma önlemi (A101 / Migros)

Hiçbir kategoriyi kaçırmamak ve **her zaman yaprak kategori bazlı** tarama yapmak için önerilen yöntemler ayrı dokümanda: **`docs/KATEGORI-KACIRMA-ONLEMI-A101-MIGROS.md`**. Özet: Şok zaten taramadan önce keşif yapıyor; Migros için taramadan önce `rest/categories` ile yaprak listesi güncellenmeli; A101 için 15 parentId ile yaprak listesi tazelenmeli + sitemap ile "yeni ana kategori" uyarısı verilmeli.
