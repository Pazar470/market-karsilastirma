    ...
    while (page <= 2) { // Limit to 2 pages for "all categories" batch to keep it fast
        try {
            ...
            for (const item of items) {
                ...
                count++;
                process.stdout.write(`\r[${new Date().toLocaleTimeString()}] [Migros] Extracted ${count} products from ${masterName}...`.padEnd(80));
            }
            page++;
        } catch (e) { break; }
    }
...
async function scrapeA101(code: string, categoryName: string, categoryPath: string, market: any) {
    ...
    try {
        ...
        for (const item of items) {
            ...
            process.stdout.write(`\r[${new Date().toLocaleTimeString()}] [A101] Extracted from ${masterName} -> ${item.name.substring(0, 30)}...`.padEnd(100));
        }
        console.log(''); // New line after category done
    } catch (e: any) { }
}
...
async function scrapeSok(code: string, categoryName: string, categoryPath: string, market: any) {
    ...
    try {
        ...
        for (const p of products) {
            ...
            process.stdout.write(`\r[${new Date().toLocaleTimeString()}] [ŞOK] Extracted from ${masterName} -> ${name.substring(0, 30)}...`.padEnd(100));
        }
        console.log(''); // New line after category done
    } catch (e) { }
}
